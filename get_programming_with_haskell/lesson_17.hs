...
-- Practial Monoids - building probability tables
type Events = [String]
type Probs = [Double]
...
-- making PTable and instnace of Show
instance Show PTable where
    show (PTable events probs) = mconcat pairs where
        pairs = zipWith showPair events probs

-- createPTable ["heads", "tails"] [0.5, 0.5]

cartCombine :: (a -> b -> c) -> [a] -> [b] -> [c]
cartCombine func l1 l2 = zipWith func newL1 cycledL2 where
    nToAdd = length l2
...
combineEvents e1 e2 = cartCombine combiner e1 e2 where
...
instance Semigroup PTable where
    ...
    (<>) (PTable e1 p1) (PTable e2 p2) = createPTable newEvents newProbs where
